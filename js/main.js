...
// {
    ...
    //シャッフルの値をqualtricsから引き継ぐ。それぞれの値は価値を表している。
    var shuffle_item1 = $('input:hidden[name="Shuffle_Item1"]').val();
    var shuffle_item2 = $('input:hidden[name="Shuffle_Item2"]').val();
    var shuffle_item3 = $('input:hidden[name="Shuffle_Item3"]').val();
    var shuffle_item4 = $('input:hidden[name="Shuffle_Item4"]').val();
    var shuffle_array = [shuffle_item1,shuffle_item2,shuffle_item3,shuffle_item4];
    console.log("乱数生成できてるのか",shuffle_item1,shuffle_item2,shuffle_item3,shuffle_item4);
    console.log("即配列",[shuffle_item1,shuffle_item2,shuffle_item3,shuffle_item4])
    console.log(shuffle_array);
    //ここで価値順にソート
    shuffle_array.sort(function(a,b){
        if( a > b ) return -1;
        if( a < b ) return 1;
        return 0;
    });
    console.log("ソート後",shuffle_array);

    for(var i=0; i<shuffle_array.length; i++){
        if(shuffle_array[i]==shuffle_item1){
            shuffle_array[i]= 0;//時計プリンハサミ
        }else if(shuffle_array[i]==shuffle_item2){
            shuffle_array[i]= 1;//ブレスレット桜餅えんぴつ
        }else if(shuffle_array[i]==shuffle_item3){
            shuffle_array[i]= 2;//イヤリングエクレア三角定規
        }else if(shuffle_array[i]==shuffle_item4){
            shuffle_array[i]= 3;//ネックレスショートケーキコンパス
        }
    }
    console.log("デコード後",shuffle_array);

    // var Shuffle_image_array = arrayShuffle(image_array[$('input:hidden[name="Array"]').val()]);
    var Shuffle_image_array = image_array[$('input:hidden[name="Array"]').val()];

    //ユーザにとって価値高い順
    var item1_image = Shuffle_image_array[shuffle_array[0]];
    var item2_image = Shuffle_image_array[shuffle_array[1]];
    var item3_image = Shuffle_image_array[shuffle_array[2]];
    var item4_image = Shuffle_image_array[shuffle_array[3]];
    


    

    for(var i=1; i<item1+1; i++){
    ...
    $(document).ready(function () {
      ...
      $("#slider1").slider({
        ...
        for(var i=1; i<item1+1; i++){$('#myitem1_'+i).hide();}
        for(var i=1; i<item1+1; i++){$('#opitem1_'+i).hide();}
        
        total = get_total();
        mytotal = get_total_point();
        //item1_count.innerHTML = ui.value+"個×"+op_item1_point+"点";
        item1_count.innerHTML = ui.value+"個×？点";
        my_item1.innerHTML = item1-ui.value+"個×"+my_item1_point+"点";
      ...
      valence5_text = item1-ui.value;


      for(var i=1; i<item1-ui.value+1; i++){$('#myitem1_'+i).show();}
      ...
      $("#slider2").slider({
        ...
        change: function (event, ui) { //スライダー変更
          ...
          for(var i=1; i<item2+1; i++){$('#myitem2_'+i).hide();}
          for(var i=1; i<item2+1; i++){$('#opitem2_'+i).hide();}

              total = get_total();
              mytotal = get_total_point();
              //item2_count.innerHTML = ui.value+"個×"+op_item2_point+"点";
              item2_count.innerHTML = ui.value+"個×？点";
              my_item2.innerHTML = item2-ui.value+"個×"+my_item2_point+"点";
            ...
            valence6_text = item2-ui.value;

            for(var i=1; i<item2-ui.value+1; i++){$('#myitem2_'+i).show();}
      ...
      $("#slider3").slider({
        ...
        change: function (event, ui) { //スライダー変更
          ...
          for(var i=1; i<item3+1; i++){$('#myitem3_'+i).hide();}
          for(var i=1; i<item3+1; i++){$('#opitem3_'+i).hide();}

              total = get_total();
              mytotal = get_total_point();
              //item3_count.innerHTML = ui.value+"個×"+op_item3_point+"点";
              item3_count.innerHTML = ui.value+"個×？点";
              my_item3.innerHTML = item3-ui.value+"個×"+my_item3_point+"点";
              // my_item3_point.innerHTML = (item3-ui.value)*my_item3_point;
              my_total_point.innerHTML = mytotal;
              //op_total_point.innerHTML = total;
              valence3 = ui.value;
            valence3_text = valence3.toFixed();
            valence7_text = item3-ui.value;

            for(var i=1; i<item3-ui.value+1; i++){$('#myitem3_'+i).show();}
       ...
       $("#slider4").slider({
         ...
         change: function (event, ui) { //スライダー変更
          ...
          for(var i=1; i<item4+1; i++){$('#myitem4_'+i).hide();}
          for(var i=1; i<item4+1; i++){$('#opitem4_'+i).hide();}
          
               total = get_total();
               mytotal = get_total_point();
               //item4_count.innerHTML = ui.value+"個×"+op_item4_point+"点";
               item4_count.innerHTML = ui.value+"個×？点";
               my_item4.innerHTML = item4-ui.value+"個×"+my_item4_point+"点";
              //  my_item4_point.innerHTML = (item4-ui.value)*my_item4_point;
              my_total_point.innerHTML = mytotal;
              //op_total_point.innerHTML = total;
               valence4 = ui.value;
             valence4_text = valence4.toFixed();
             valence8_text = item4-ui.value;

             for(var i=1; i<item4-ui.value+1; i++){$('#myitem4_'+i).show();}
    ...
    var i_array_1 = [ //blink pattern A
        ...
        "https://rc1userv5pwvgnvtxbwj.au1.qualtrics.com/ControlPanel/Graphic.php?IM=IM_08QVyU8BIV1SEM6",  // 22個にするためにセンターを増やす 
    ...
    function faceDraw(yy, fact01, fact02) { //表情を描画する //yy=金額, fact01=表情, fact02=まばたき
      ...
      //yの変換  
      var sumitem = item1*op_item1_point + item2*op_item2_point + item3*op_item3_point + item4*op_item4_point;
    ...
    function get_total_point(){
      var total_point = (item1-$("#slider1").slider("option","value"))*my_item1_point+(item2-$("#slider2").slider("option","value"))*my_item2_point+(item3-$("#slider3").slider("option","value"))*my_item3_point+(item4-$("#slider4").slider("option","value"))*my_item4_point;
